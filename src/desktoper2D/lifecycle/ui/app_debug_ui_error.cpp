...
void RenderRuntimePluginQuickControlPanel(AppRuntime &runtime) {
    ...
    if (ImGui::Button("刷新##behavior_plugins")) {
        ApplyPluginAction(runtime, PluginAction{.type = PluginActionType::RefreshPluginConfigs});
    }
    ...
    auto open_detail = [&](PluginDetailKind kind,
        ...
        runtime.plugin.detail_kind = kind;
        runtime.plugin.detail_title = title ? title : "";
        runtime.plugin.detail_source = source;
        runtime.plugin.detail_assets = assets;
        runtime.plugin.detail_backend = backend;
        runtime.plugin.detail_status = status;
        runtime.plugin.detail_last_error = last_error;
        runtime.plugin.detail_edit_loaded = false;
        runtime.plugin.detail_edit_source = source;
        runtime.workspace_ui.panels.show_plugin_detail_window = true;
        runtime.workspace_ui.panels.show_plugin_quick_control_window = false;
    };
    ...
    if (!runtime.plugin.config_entries.empty()) {
        for (std::size_t i = 0; i < runtime.plugin.config_entries.size(); ++i) {
            auto &entry = runtime.plugin.config_entries[i];
            ...
            if (i < 2) {
                ...
                if (RenderFilePickerButton("切换", &picked_path, "ONNX Model", "onnx", "", &pick_err)) {
                    ApplyPluginAction(runtime,
                                      PluginAction{
                                          .type = PluginActionType::ReplaceUnifiedPluginAssets,
                                          .feedback_slot = PluginActionFeedbackSlot::Switch,
                                          .text_value = std::string("behavior:") + entry.config_path,
                                          .text_value_2 = picked_path,
                                      });
                } else if (!pick_err.empty()) {
                    runtime.plugin.switch_status.clear();
                    runtime.plugin.switch_error = pick_err;
                }
            } else {
                if (ImGui::Button("切换")) {
                    SDL_strlcpy(runtime.plugin.name_input, entry.name.c_str(), sizeof(runtime.plugin.name_input));
                    ApplyPluginAction(runtime,
                                      PluginAction{.type = PluginActionType::SwitchPluginByName,
                                                   .text_value = runtime.plugin.name_input});
                }
            }
            if (!entry_enabled_now) {
                ImGui::EndDisabled();
            ...
            ImGui::PopID();
            ImGui::PopID();
            if (i + 1 < runtime.plugin.config_entries.size()) {
    ...
    if (pending_delete_requested && !pending_delete_path.empty()) {
        ApplyPluginAction(runtime,
                          PluginAction{.type = PluginActionType::DeletePluginConfig,
                                       .text_value = pending_delete_path});
    }
    ...
    auto render_plugin_like_card = [&](const char *title,
        ...
        const DefaultPluginCatalogEntry *asr_spec = FindDefaultPluginCatalogEntry(runtime, "asr");
        const DefaultPluginCatalogEntry *ocr_spec = FindDefaultPluginCatalogEntry(runtime, "ocr");
        const DefaultPluginCatalogEntry *scene_spec = FindDefaultPluginCatalogEntry(runtime, "mobileclip");
        const DefaultPluginCatalogEntry *facemesh_spec = FindDefaultPluginCatalogEntry(runtime, "facemesh");
        const DefaultPluginCatalogEntry *chat_spec = FindDefaultPluginCatalogEntry(runtime, "chat");
        const bool has_asr_spec = asr_spec != nullptr;
        const bool has_ocr_spec = ocr_spec != nullptr;
        const bool has_scene_spec = scene_spec != nullptr;
        const bool has_facemesh_spec = facemesh_spec != nullptr;
        const bool has_chat_spec = chat_spec != nullptr;

        if (runtime.plugin.asr_provider_entries.empty()) {
            ApplyPluginAction(runtime, PluginAction{.type = PluginActionType::RefreshAsrProviders});
        }
        std::string asr_provider_name = runtime.plugin.asr_current_provider_name;
        if (asr_provider_name.empty() &&
            runtime.plugin.asr_selected_entry_index >= 0 &&
            runtime.plugin.asr_selected_entry_index < static_cast<int>(runtime.plugin.asr_provider_entries.size())) {
            asr_provider_name = runtime.plugin.asr_provider_entries[static_cast<std::size_t>(runtime.plugin.asr_selected_entry_index)].name;
        }
        ...
        if (render_plugin_like_card(
                "ASR",
                runtime.feature_flags.asr_enabled,
                asr_detail.c_str(),
                runtime.asr_last_error,
                [&](bool enabled_flag) { runtime.feature_flags.asr_enabled = enabled_flag; },
                [&]() {
                    ...
                    if (RenderFilePickerButton("切换##asr", &asr_picked_path, "ONNX Model", "onnx", "", &asr_pick_err)) {
                        runtime.plugin.override_asr_model_path = asr_picked_path;
                        ApplyPluginAction(runtime,
                                          PluginAction{
                                              .type = PluginActionType::ApplyOverrideModels,
                                              .feedback_slot = PluginActionFeedbackSlot::AsrSwitch,
                                              .text_value = "asr model override ok",
                                              .text_value_2 = "asr override failed",
                                              .text_value_3 = "asr",
                                          });
                    } else if (!asr_pick_err.empty()) {
                        runtime.plugin.asr_switch_status.clear();
                        runtime.plugin.asr_switch_error = asr_pick_err;
                    }
                })) {
            std::vector<std::string> assets;
            if (!runtime.plugin.override_asr_model_path.empty()) {
                assets.push_back(runtime.plugin.override_asr_model_path);
        ...
        if (runtime.plugin.ocr_model_entries.empty()) {
            ApplyPluginAction(runtime, PluginAction{.type = PluginActionType::RefreshOcrModels});
        }
        ...
        if (runtime.plugin.ocr_selected_entry_index >= 0 &&
            ...
            ocr_model_name = runtime.plugin.ocr_model_entries[static_cast<std::size_t>(runtime.plugin.ocr_selected_entry_index)].name;
        }
        if (ocr_model_name.empty() && runtime.plugin.ocr_model_input[0] != '\0') {
            ocr_model_name = runtime.plugin.ocr_model_input;
        }
        ...
        if (runtime.plugin.ocr_selected_entry_index >= 0 &&
            runtime.plugin.ocr_selected_entry_index < static_cast<int>(runtime.plugin.ocr_model_entries.size())) {
            base_ocr = &runtime.plugin.ocr_model_entries[static_cast<std::size_t>(runtime.plugin.ocr_selected_entry_index)];
        } else if (!runtime.plugin.ocr_model_entries.empty()) {
            base_ocr = &runtime.plugin.ocr_model_entries.front();
        }

        ImGui::PushID("ocr_plugin_card");
        if (render_plugin_like_card(
                "OCR",
                runtime.feature_flags.ocr_enabled,
                ocr_detail.c_str(),
                runtime.perception_state.ocr_last_error,
                [&](bool enabled_flag) { runtime.feature_flags.ocr_enabled = enabled_flag; },
                [&]() {
                    ...
                    if (RenderFilePickerButton("切换##ocr", &ocr_picked_path, "ONNX Model", "onnx", "", &ocr_pick_err)) {
                        if (base_ocr == nullptr) {
                            ...
                            runtime.plugin.ocr_switch_error = "ocr model base not available";
                        } else {
                            runtime.plugin.override_ocr_det_path = ocr_picked_path;
                            runtime.plugin.override_ocr_rec_path = base_ocr->rec_path;
                            runtime.plugin.override_ocr_keys_path = base_ocr->keys_path;
                            ApplyPluginAction(runtime,
                                              PluginAction{
                                                  .type = PluginActionType::ApplyOverrideModels,
                                                  .feedback_slot = PluginActionFeedbackSlot::OcrSwitch,
                                                  .text_value = "ocr model override ok",
                                                  .text_value_2 = "ocr override failed",
                                                  .text_value_3 = "ocr",
                                              });
                        }
                    } else if (!ocr_pick_err.empty()) {
                        runtime.plugin.ocr_switch_status.clear();
                        runtime.plugin.ocr_switch_error = ocr_pick_err;
                    }
                })) {
            std::vector<std::string> assets;
            assets.reserve(3);
            const std::string det_path = !runtime.plugin.override_ocr_det_path.empty()
                                             ? runtime.plugin.override_ocr_det_path
                                             : (has_ocr_spec && !ocr_spec->det.empty()
                                                    ? ocr_spec->det
                                                    : (base_ocr ? base_ocr->det_path : std::string()));
            const std::string rec_path = !runtime.plugin.override_ocr_rec_path.empty()
                                             ? runtime.plugin.override_ocr_rec_path
                                             : (has_ocr_spec && !ocr_spec->rec.empty()
                                                    ? ocr_spec->rec
                                                    : (base_ocr ? base_ocr->rec_path : std::string()));
            const std::string keys_path = !runtime.plugin.override_ocr_keys_path.empty()
        ...
        const std::string scene_detail = build_model_detail(runtime.plugin.override_scene_model_path,
                                                            scene_default_model.c_str(),
                                                            "Model");
        const std::string scene_labels_detail = build_model_detail(runtime.plugin.override_scene_labels_path,
        ...
        if (render_plugin_like_card(
                "MobileCLIP",
                runtime.feature_flags.scene_classifier_enabled,
                scene_full_detail.c_str(),
                runtime.perception_state.scene_classifier_last_error,
                [&](bool enabled_flag) { runtime.feature_flags.scene_classifier_enabled = enabled_flag; },
                [&]() {
                    ...
                    if (RenderFilePickerButton("模型##scene", &scene_model_picked_path, "ONNX Model", "onnx", "", &scene_model_pick_err)) {
                        runtime.plugin.override_scene_model_path = scene_model_picked_path;
                        ApplyPluginAction(runtime,
                                          PluginAction{
                                              .type = PluginActionType::ApplyOverrideModels,
                                              .feedback_slot = PluginActionFeedbackSlot::OverrideApply,
                                              .text_value = "scene model override ok",
                                              .text_value_2 = "scene override failed",
                                              .text_value_3 = "scene",
                                          });
                    } else if (!scene_model_pick_err.empty()) {
                        runtime.plugin.override_apply_status.clear();
                        runtime.plugin.override_apply_error = scene_model_pick_err;
                    }
                    ImGui::SameLine();
                    ...
                    if (RenderFilePickerButton("标签##scene", &scene_labels_picked_path, "Labels", "json", "", &scene_labels_pick_err)) {
                        runtime.plugin.override_scene_labels_path = scene_labels_picked_path;
                        ApplyPluginAction(runtime,
                                          PluginAction{
                                              .type = PluginActionType::ApplyOverrideModels,
                                              .feedback_slot = PluginActionFeedbackSlot::OverrideApply,
                                              .text_value = "scene labels override ok",
                                              .text_value_2 = "scene override failed",
                                              .text_value_3 = "scene",
                                          });
                    } else if (!scene_labels_pick_err.empty()) {
                        runtime.plugin.override_apply_status.clear();
                        runtime.plugin.override_apply_error = scene_labels_pick_err;
                    }
                })) {
            std::vector<std::string> assets;
            assets.push_back(!runtime.plugin.override_scene_model_path.empty() ? runtime.plugin.override_scene_model_path : scene_default_model);
            assets.push_back(!runtime.plugin.override_scene_labels_path.empty() ? runtime.plugin.override_scene_labels_path : scene_default_labels);
        ...
        const std::string face_detail = build_model_detail(runtime.plugin.override_facemesh_model_path,
                                                           face_default_model.c_str(),
                                                           "Model");
        const std::string face_labels_detail = build_model_detail(runtime.plugin.override_facemesh_labels_path,
        ...
        if (render_plugin_like_card(
                "FaceMesh",
                runtime.feature_flags.face_emotion_enabled,
                face_full_detail.c_str(),
                runtime.perception_state.camera_facemesh_last_error,
                [&](bool enabled_flag) { runtime.feature_flags.face_emotion_enabled = enabled_flag; },
                [&]() {
                    ...
                    if (RenderFilePickerButton("模型##facemesh", &face_model_picked_path, "ONNX Model", "onnx", "", &face_model_pick_err)) {
                        runtime.plugin.override_facemesh_model_path = face_model_picked_path;
                        ApplyPluginAction(runtime,
                                          PluginAction{
                                              .type = PluginActionType::ApplyOverrideModels,
                                              .feedback_slot = PluginActionFeedbackSlot::OverrideApply,
                                              .text_value = "facemesh model override ok",
                                              .text_value_2 = "facemesh override failed",
                                              .text_value_3 = "facemesh",
                                          });
                    } else if (!face_model_pick_err.empty()) {
                        runtime.plugin.override_apply_status.clear();
                        runtime.plugin.override_apply_error = face_model_pick_err;
                    }
                    ImGui::SameLine();
                    ...
                    if (RenderFilePickerButton("标签##facemesh", &face_labels_picked_path, "Labels", "json", "", &face_labels_pick_err)) {
                        runtime.plugin.override_facemesh_labels_path = face_labels_picked_path;
                        ApplyPluginAction(runtime,
                                          PluginAction{
                                              .type = PluginActionType::ApplyOverrideModels,
                                              .feedback_slot = PluginActionFeedbackSlot::OverrideApply,
                                              .text_value = "facemesh labels override ok",
                                              .text_value_2 = "facemesh override failed",
                                              .text_value_3 = "facemesh",
                                          });
                    } else if (!face_labels_pick_err.empty()) {
                        runtime.plugin.override_apply_status.clear();
                        runtime.plugin.override_apply_error = face_labels_pick_err;
                    }
                })) {
            std::vector<std::string> assets;
            assets.push_back(!runtime.plugin.override_facemesh_model_path.empty() ? runtime.plugin.override_facemesh_model_path : face_default_model);
            assets.push_back(!runtime.plugin.override_facemesh_labels_path.empty() ? runtime.plugin.override_facemesh_labels_path : face_default_labels);
        ...
        if (render_plugin_like_card(
                "Chat",
                runtime.feature_flags.chat_enabled,
                chat_detail.c_str(),
                runtime.chat_last_error,
                [&](bool enabled_flag) { runtime.feature_flags.chat_enabled = enabled_flag; },
                [&]() {
        ...
        if (ImGui::Button("Create")) {
            runtime.plugin.show_unified_create_modal = true;
        }

        ImGui::EndChild();

    if (runtime.plugin.show_unified_create_modal) {
        ImGui::OpenPopup("Create Plugin");
        runtime.plugin.show_unified_create_modal = false;
    }
    if (ImGui::BeginPopupModal("Create Plugin", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
        ImGui::Text("New Plugin");
        ImGui::Separator();
        ImGui::InputTextWithHint("##new_plugin_name", "新建插件名称", runtime.plugin.unified_new_name_input,
                                 sizeof(runtime.plugin.unified_new_name_input));
        ImGui::Dummy(ImVec2(0.0f, 6.0f));
        if (ImGui::Button("Create")) {
            ApplyPluginAction(runtime,
                              PluginAction{
                                  .type = PluginActionType::CreateUserPlugin,
                                  .feedback_slot = PluginActionFeedbackSlot::UnifiedCreate,
                                  .bool_value = true,
                                  .text_value = runtime.plugin.unified_new_name_input,
                              });
    ...
    if (!runtime.plugin.override_apply_status.empty()) {
        ImGui::TextColored(ImVec4(0.45f, 0.85f, 0.45f, 1.0f), "%s", runtime.plugin.override_apply_status.c_str());
    }
    if (!runtime.plugin.override_apply_error.empty()) {
        ImGui::TextColored(ImVec4(1.0f, 0.6f, 0.3f, 1.0f), "%s", runtime.plugin.override_apply_error.c_str());
    }
    if (!runtime.plugin.unified_switch_status.empty()) {
        ImGui::TextColored(ImVec4(0.45f, 0.85f, 0.45f, 1.0f), "%s", runtime.plugin.unified_switch_status.c_str());
    }
    if (!runtime.plugin.unified_switch_error.empty()) {
        ImGui::TextColored(ImVec4(1.0f, 0.6f, 0.3f, 1.0f), "%s", runtime.plugin.unified_switch_error.c_str());
    }
    if (!runtime.plugin.unified_create_status.empty()) {
        ImGui::TextColored(ImVec4(0.45f, 0.85f, 0.45f, 1.0f), "%s", runtime.plugin.unified_create_status.c_str());
    }
    if (!runtime.plugin.unified_create_error.empty()) {
        ImGui::TextColored(ImVec4(1.0f, 0.6f, 0.3f, 1.0f), "%s", runtime.plugin.unified_create_error.c_str());
    }
    if (!runtime.plugin.unified_delete_status.empty()) {
        ImGui::TextColored(ImVec4(0.45f, 0.85f, 0.45f, 1.0f), "%s", runtime.plugin.unified_delete_status.c_str());
    }
    if (!runtime.plugin.unified_delete_error.empty()) {
        ImGui::TextColored(ImVec4(1.0f, 0.6f, 0.3f, 1.0f), "%s", runtime.plugin.unified_delete_error.c_str());
    }

...
void TickEditor(AppRuntime &runtime, EditorStateSlice editor, float dt, const RuntimeTickBridge &bridge) {
    if (!editor.editor_autosave_recovery_checked) {
        RefreshEditorAutosaveState(runtime);
    }

    if (editor.editor_autosave_enabled && editor.model_loaded) {
        if (!editor.editor_project_dirty) {
            editor.editor_autosave_accum_sec = 0.0f;
        } else {
            editor.editor_autosave_accum_sec += std::max(0.0f, dt);
            if (editor.editor_autosave_accum_sec >= editor.editor_autosave_interval_sec) {
                std::string err;
                if (SaveEditorAutosaveProject(runtime, &err)) {
                    editor.editor_status = "autosaved project";
                    editor.editor_status_ttl = 2.0f;
                } else {
                    editor.editor_status = "autosave failed: " + err;
                    editor.editor_status_ttl = 3.5f;
                }
                editor.editor_autosave_accum_sec = 0.0f;
            }
        }
    ...
    UpdateHeadPatReaction(
        editor.interaction_state,
        InteractionControllerContext{
            .model_loaded = editor.model_loaded,
            .model = &editor.model,
            .pick_top_part_at = bridge.pick_top_part_at,
            .has_model_params = bridge.has_model_params,
        },
        dt);
    UpdateModelRuntime(&editor.model, editor.model_time, dt);
}

void TickDecision(PerceptionStateSlice perception, PluginStateSlice plugin, float dt) {
    RefreshTaskDecisionInputCache(perception.pipeline_state);
    const OcrResult &ocr_for_decision = GetTaskDecisionOcrInput(perception.pipeline_state);
    TickTaskDecision(perception.pipeline_state.system_context_snapshot,
                     ocr_for_decision,
                     perception.pipeline_state.scene_result,
                     perception.pipeline_state.decision_signature.input_signature,
                     dt,
                     plugin.task_decision.config,
                     plugin.task_decision.schedule,
                     plugin.task_decision.inference_state,
                     &perception.pipeline_state.blackboard.asr.session_text,
                     plugin.task_decision.last_result,
                     &plugin.task_decision.error_info);
    plugin.task_decision.primary = plugin.task_decision.last_result.primary;
    plugin.task_decision.secondary = plugin.task_decision.last_result.secondary;
}
...
void TickBehavior(EditorStateSlice editor,
    ...
    if (perception.feature_flags.face_param_mapping_enabled) {
        auto &fm = perception.pipeline_state.face_emotion_result;

        perception.face_map_gate_reason = "ok";
        perception.face_map_sensor_fallback_active = false;
        perception.face_map_sensor_fallback_reason = "none";
        perception.face_map_raw_yaw_deg = fm.head_yaw_deg;
        perception.face_map_raw_pitch_deg = fm.head_pitch_deg;
        perception.face_map_raw_eye_open = fm.eye_open_avg;

        const float min_conf = std::clamp(perception.face_map_min_confidence, 0.0f, 1.0f);
        const float deadzone_deg = std::max(0.0f, perception.face_map_head_pose_deadzone_deg);
        const float yaw_max_deg = std::max(1.0f, perception.face_map_yaw_max_deg);
        const float pitch_max_deg = std::max(1.0f, perception.face_map_pitch_max_deg);
        const float eye_thr = std::clamp(perception.face_map_eye_open_threshold, 0.0f, 1.0f);
        const float w = std::clamp(perception.face_map_param_weight, 0.0f, 1.0f);
        const float smooth_alpha = std::clamp(perception.face_map_smooth_alpha, 0.0f, 1.0f);
        ...
        if (!fm.face_detected) {
            perception.face_map_gate_reason = "no_face";
            gate_ok = false;
            fallback_requested = true;
            fallback_reason = "no_face";
        } else if (fm.emotion_score < min_conf) {
            perception.face_map_gate_reason = "low_conf";
            gate_ok = false;
            fallback_requested = true;
        ...
        if (!std::isfinite(fm.head_yaw_deg) || !std::isfinite(fm.head_pitch_deg) || !std::isfinite(fm.eye_open_avg)) {
            perception.face_map_gate_reason = "nan_input";
            gate_ok = false;
            fallback_requested = true;
            fallback_reason = "nan_input";
        }

        if (perception.pipeline_state.facemesh_error_info.code != RuntimeErrorCode::Ok) {
            fallback_requested = true;
        ...
        if (fallback_requested) {
            if (perception.face_map_sensor_fallback_enabled) {
                const float fw = std::clamp(perception.face_map_sensor_fallback_weight, 0.0f, 1.0f);
                const float fyaw = std::clamp(perception.face_map_sensor_fallback_head_yaw, -1.0f, 1.0f);
                const float fpitch = std::clamp(perception.face_map_sensor_fallback_head_pitch, -1.0f, 1.0f);
                const float feye = std::clamp(perception.face_map_sensor_fallback_eye_open, 0.0f, 1.0f);

                mapped_yaw = mapped_yaw * (1.0f - fw) + fyaw * fw;
                mapped_pitch = mapped_pitch * (1.0f - fw) + fpitch * fw;
                mapped_eye = mapped_eye * (1.0f - fw) + feye * fw;

                perception.face_map_sensor_fallback_active = fw > 0.0f;
                perception.face_map_sensor_fallback_reason = fallback_reason;
            } else {
                perception.face_map_sensor_fallback_reason = "disabled";
            }
        }

        perception.face_map_out_head_yaw =
            perception.face_map_out_head_yaw * (1.0f - smooth_alpha) + mapped_yaw * smooth_alpha;
        perception.face_map_out_head_pitch =
            perception.face_map_out_head_pitch * (1.0f - smooth_alpha) + mapped_pitch * smooth_alpha;
        perception.face_map_out_eye_open =
            perception.face_map_out_eye_open * (1.0f - smooth_alpha) + mapped_eye * smooth_alpha;

        auto blend_target = [&](const char *pid, float norm, float min_value, float max_value) {
            const auto it = editor.model.param_index.find(pid);
            if (it == editor.model.param_index.end()) return;
            const int idx = it->second;
            if (idx < 0 || idx >= static_cast<int>(editor.model.parameters.size())) return;

            auto &param = editor.model.parameters[static_cast<std::size_t>(idx)].param;
        ...
        blend_target("HeadYaw", perception.face_map_out_head_yaw, -1.0f, 1.0f);
        blend_target("HeadPitch", perception.face_map_out_head_pitch, -1.0f, 1.0f);
        ...
        auto blend_eye = [&](const char *pid) {
            ...
            auto &param = editor.model.parameters[static_cast<std::size_t>(idx)].param;
            const float target = std::clamp(perception.face_map_out_eye_open, 0.0f, 1.0f);
            const float cur = param.target();
            const float mixed = cur * (1.0f - w) + target * w;
    ...
    plugin.plugin_runtime.route_selected = "unknown";
    plugin.plugin_runtime.route_scene_score = 0.0;
    plugin.plugin_runtime.route_task_score = 0.0;
    plugin.plugin_runtime.route_presence_score = 0.0;
    plugin.plugin_runtime.route_total_score = 0.0;
    plugin.plugin_runtime.route_rejected_summary.clear();
    if (plugin.plugin_runtime.ready && plugin.feature_flags.plugin_enabled) {
        const TaskCategoryInferenceResult &task_result = plugin.task_decision.last_result;

        PerceptionInput in{};
        in.time_sec = static_cast<double>(plugin.model_time);
        in.audio_level = 0.0f;
        in.user_presence = perception.pipeline_state.face_emotion_result.face_detected
                               ? 1.0f
                               : (plugin.window_state.window_visible ? 1.0f : 0.0f);
        in.vision.user_presence = in.user_presence;
        in.vision.head_yaw_deg = perception.pipeline_state.face_emotion_result.head_yaw_deg;
        in.vision.head_pitch_deg = perception.pipeline_state.face_emotion_result.head_pitch_deg;
        in.vision.head_roll_deg = perception.pipeline_state.face_emotion_result.head_roll_deg;
        in.vision.gaze_x = 0.0f;
        in.vision.gaze_y = 0.0f;
        in.state.window_visible = plugin.window_state.window_visible;
        in.state.click_through = plugin.window_state.click_through;
        in.state.manual_param_mode = plugin.manual_param_mode;
        in.state.show_debug_stats = plugin.show_debug_stats;
        in.state.dt_sec = dt;
        in.scene_label = perception.pipeline_state.scene_result.label;
        in.task_label = bridge.TaskSecondaryCategoryName(plugin.task_decision.secondary);
        ...
        plugin.plugin_runtime.inference_adapter->SubmitInput(std::move(in));
        has_plugin_out = plugin.plugin_runtime.inference_adapter->TryConsumeLatestOutput(plugin_out, nullptr);
        if (has_plugin_out) {
            ...
            for (const auto &kv : plugin_out.event_scores) {
                ...
                if (key == kSceneScoreKey) {
                    plugin.plugin_runtime.route_scene_score = value;
                    continue;
                }
                if (key == kTaskScoreKey) {
                    plugin.plugin_runtime.route_task_score = value;
                    continue;
                }
                if (key == kPresenceScoreKey) {
                    plugin.plugin_runtime.route_presence_score = value;
                    continue;
                }
                if (key == kTotalScoreKey) {
                    plugin.plugin_runtime.route_total_score = value;
                    continue;
                }
                ...
                if (key.rfind(kRejectedPrefix, 0) == 0) {
                    ...
                    if (!has_score_suffix) {
                        plugin.plugin_runtime.route_rejected_summary.push_back(key + "=" + std::to_string(value));
                    }
                    continue;
                }
            ...
            plugin.plugin_runtime.route_selected = selected;
        }
    } else if (!plugin.feature_flags.plugin_enabled) {
        plugin.plugin_runtime.route_selected = "disabled";
    }
    ...
    if (MixBehaviorOutputs(mix_sources, plugin.plugin_runtime.behavior_fusion_config, &mix_result) &&
...
void RunRuntimeTickEntry(AppRuntime &runtime, float dt, const RuntimeTickBridge &bridge) {
    ...
    if (slices.plugin.plugin_runtime.ready && slices.plugin.feature_flags.plugin_enabled) {
    ...
    if (!slices.plugin.plugin_runtime.ready) {
        TickDecision(slices.perception, slices.plugin, dt);
    }
}

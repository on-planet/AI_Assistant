...
void NormalizeTimelineSelection(AppRuntime &runtime, AnimationChannel &ch) {
    k2d::EnsureTimelineKeyframeStableIds(ch);
    ...
    runtime.timeline_selected_keyframe_indices = k2d::BuildTimelineSelectedIndices(ch, runtime.timeline_selected_keyframe_ids);
}
...
void SelectTimelineKeyframeById(AppRuntime &runtime, AnimationChannel &ch, std::uint64_t stable_id, bool additive) {
    ...
    k2d::NormalizeTimelineSelection(runtime, ch);
}
...
bool TimelineKeyframeListEqual(const std::vector<TimelineKeyframe> &lhs,
    ...
    for (std::size_t i = 0; i < lhs.size(); ++i) {
        if (!k2d::TimelineKeyframeNearlyEqual(lhs[i], rhs[i])) {
            return false;
        }
    }
    return true;
...
void PushTimelineEditCommand(AppRuntime &runtime,
                             ...
                             const std::vector<std::uint64_t> &before_selected_ids,
                             const std::vector<std::uint64_t> &after_selected_ids) {
    if (k2d::TimelineKeyframeListEqual(before_keyframes, after_keyframes) && before_selected_ids == after_selected_ids) {
        return;
    }
    PushEditCommand(runtime.undo_stack,
                    runtime.redo_stack,
                    k2d::MakeTimelineEditCommand(channel,
                                            before_keyframes,
                                            after_keyframes,
                                            before_selected_ids,
                                            after_selected_ids));
